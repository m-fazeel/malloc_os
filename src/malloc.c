...
void* malloc(size_t size)
{
   ...
   /* TODO: If the block found by findFreeBlock is larger than we need then:
      ...
      TODO: increment the counter for the number of splits
   */
   ...
   if (next != NULL)
   {
      ...
      {
         struct _block* newblock = (struct _block*)sbrk(sizeof(struct _block) + size);
...
void* calloc(size_t nmemb, size_t size)
{
   ...
   struct _block* allocate = malloc(nmemb * size);
   if (allocate != NULL)
   {
      memset(allocate, 0, nmemb * size);
      return allocate;
   }
...
void* realloc(void* ptr, size_t size)
{
   ...
   struct _block* reallocate = malloc(size);
   if (reallocate != NULL)
   {
